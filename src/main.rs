...
impl Score for BlackjackScore {
    ...
    fn score(&self, hand: &Hand) -> Self::Output {
        let score = hand
            .cards()
            .into_iter()
            .map(|c| ((*c.rank() as u8) + 1).min(10))
            .sum();

        if score <= 11 && hand.cards().into_iter().any(|c| c.rank() == &Rank::Ace) {
            (score, Some(score + 10))
        } else {
            (score, None)
        }
...
impl<'a> Event<'a> {
    pub fn player(player: usize, event: PlayerEvent<'a>) -> Self {
        Self::Player { player, event }
    }

    pub fn dealer(event: DealerEvent<'a>) -> Self {
        Self::Dealer(event)
    }

    pub fn win(event: WinEvent) -> Self {
        Self::Win(event)
    }
}
    ...
    D: Deck,
{
    ...
    pub fn score(&self, player: usize) -> Option<<BlackjackScore as Score>::Output> {
        ...
        E: for<'a> FnMut(Event<'a>),
    {
        ...
        let player_blackjacks = (0..self.hands.len())
            .filter(|&player| self.hands[player].score(&self.score).1 == Some(21))
            .collect::<Vec<_>>();
        ...
        if player_blackjacks.is_empty() {
            if dealer_blackjack {
                ...
                event(Event::win(WinEvent::Lose((0..self.hands.len()).collect())));
                return;
            }
        } else {
            ...
            if dealer_blackjack {
                event(Event::win(WinEvent::Draw(player_blackjacks)));
                event(Event::win(WinEvent::Lose(losers)));
                return;
            } else {
                event(Event::win(WinEvent::Players(player_blackjacks)));
                event(Event::win(WinEvent::Lose(losers)));
                return;
            }
        }
        ...
        for player in 0..self.hands.len() {
            ...
            while !self.hands[player].is_bust(&self.score) {
                ...
                decisions += 1;
                event(Event::player(player, PlayerEvent::ChooseDecision(decision)));
                        ...
                        if decisions == 1 {
                            ...
                            event(Event::player(
                                ...
                                PlayerEvent::InvalidDecision {
                                    invalid_decision: decision,
                                    reason: Cow::Borrowed(
                                        "Can only surrender at the start of a game",
                                    ),
                                },
                            ));
                        }
            ...
            if self.hands[player].is_bust(&self.score) {
                ...
                event(Event::player(player, PlayerEvent::Bust { score: score.0 }));
                out_of_the_game.push(player);
            }
        ...
        if winner.is_empty() {
            if draws.is_empty() {
                ...
                event(Event::win(WinEvent::Draw(draws)));
            }
        } else {
...
fn main() {
    ...
    bjack.run(
        |player, game| {
            if player != last_player {
                ...
                println!(
                    ...
                    game.player(player)
                        ...
                        .join(" "),
                    score.0,
                    score.1.map(|s| format!("/{}", s)).unwrap_or(String::new())
                );
                last_player = player;
            }
            ...
            loop {
                println!("Options [H]it, [S]tand, [D]oubleDown, S[P]lit, Su[R]render]");
        ...
        |event| {
            match event {
                Event::Player { player, event } => match event {
                    PlayerEvent::TurnStart => println!("--- >> Player {} << ---", player),
                    PlayerEvent::TurnEnd => println!("-------------------------"),
                    PlayerEvent::ChooseDecision(_decision) => {}
                    PlayerEvent::ExecuteDecision(_decision) => {}
                    PlayerEvent::InvalidDecision {
                        invalid_decision: _invalid_decision,
                        reason,
                    } => println!("!! Player {} invalid decision: {}", player, reason),
                    PlayerEvent::Draw(card) => println!(">> Player {} drew {}", player, card),
                    ...
                    PlayerEvent::Bust { score } => {
                        println!(">> Player {} went bust ({:?})", player, score)
                    }
                    PlayerEvent::Surrender => println!(">> Player {} surrendered", player),
                    PlayerEvent::Blackjack => println!(">> Player {} got a blackjack", player),
                },
                Event::Dealer(event) => match event {
                    DealerEvent::TurnStart => println!("--- >> Dealer << ---"),
                    DealerEvent::TurnEnd => println!("-------------------------"),
                    DealerEvent::Decision(decision) => println!(">> Dealer chose to {}", decision),
                    DealerEvent::Draw(card) => println!(">> Dealer drew {}", card),
                    ...
                    DealerEvent::Bust { score } => println!(">> Dealer went bust ({:?})", score),
                    DealerEvent::Blackjack => println!(">> Dealer got a blackjack"),
                },
                Event::Win(event) => match event {
                    WinEvent::Dealer => println!("$$ Dealer won"),
                    WinEvent::Players(players) => println!("$$ Players {:?} won", players),
                    WinEvent::Draw(players) => println!("$$ Dealer and Players {:?} draw", players),
                    WinEvent::Lose(players) => println!("$$ Players {:?} lose", players),
                },
            };
        },
    );
